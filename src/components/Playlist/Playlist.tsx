...
const Playlist = (props: Props) => {
    ...
    const filteredPlaylist = currentCategory === 'all' ? PLAYLIST_DATA : PLAYLIST_DATA.filter(playlist => playlist.category === PLAYLIST_CATEGORIES[currentCategory]);
  return (
    <>
        <Box mb={8}><Text as="h1" fontFamily="heading" fontSize={6}>Playlist</Text></Box>
        <Box mb={4}>
            <Stack style={{alignItems:"center"}}>
                <Box><Text>Sort by</Text></Box>
                <button onClick={() => handleCategoryChange('all')} className={playlistCategoryStyles} style={{opacity}}><Text as="h3" fontFamily="heading" fontSize={2}>All</Text></button>
                
                {Object.entries(PLAYLIST_CATEGORIES as Record<string, string>).map(([key, value]) => 
                    <button key={value} onClick={() => handleCategoryChange(key)}  className={playlistCategoryStyles} style={{opacity: currentCategory === key ? 1 : OPACITY_UNSELECTED}}><Text as="h3" fontFamily="heading" fontSize={2}>{value}</Text></button>
                )}
            </Stack>
        </Box>
        <Stack vertical>
